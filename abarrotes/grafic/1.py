...
from ventanaUsuario import crearSecundaria

def modificarSecundaria(ventanaSecundaria):
    ventanaSecundaria.title("ABARROTES D MERLIN")  # Cambia el título
    
def iniciarApp():    
    raiz=Tk()
    raiz.title("**INICIO: ABARROTES DON MERLIN**") ##Titulo
    raiz.resizable(False,False) ##redimensionar
    ...
    botonIngU = Button(frameUser,text="Ingresar", command=lambda: [crearSecundaria(raiz),raiz.withdraw()])
